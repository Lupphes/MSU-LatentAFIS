    ...
    for finger_path in finger_paths:
        ...
        for img_file in img_files:
            ...
            matrix = np.concatenate(
                (np.expand_dims(img_STFT, axis=2), np.expand_dims(img, axis=2)), 2)
...
def get_args():
    ...
    parser.add_argument('--data', help='a jpeg directory',
                        default='/AutomatedLatentRecognition/Data/enhancement_training/')
    parser.add_argument(
...
def test_noise():
    ...
    noise = scale * \
        cv2.getGaborKernel((opt.SHAPE, opt.SHAPE), sigma, theta, lambd, gamma)
    noise = shift(noise, np.random.randint(64), cval=0.)
    # , sigma, theta,lambd,gamma)
    im_input[:, :, 0] += noise[:opt.SHAPE, :opt.SHAPE]
